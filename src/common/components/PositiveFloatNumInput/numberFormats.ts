...
export const percent = (v: number | string, maxDecimals: number | undefined = 2, hasSign = false): string => {
  if (typeof v === 'string') {
  ...
  return (hasSign && v > 0 ? '+' : '') + cutDecimals(avoidScientificNotation(v * 100), maxDecimals) + '%';
};
